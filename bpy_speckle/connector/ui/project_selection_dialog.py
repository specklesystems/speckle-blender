...
class SPECKLE_OT_project_selection_dialog(bpy.types.Operator):
    ...
    def invoke(self, context: Context, event: Event) -> set[str]:
        ...
        if wm.selected_workspace_id == "":
            workspace = get_active_workspace(selected_account_id)
            wm.selected_workspace_id = workspace[0]
            wm.selected_workspace_name = workspace[1]

        # Fetch projects from server
        projects: List[Tuple[str, str, str, str, bool]] = get_projects_for_account(
            selected_account_id, workspace_id=wm.selected_workspace_id
        )
