"""Map a selection of Blender objects onto a Speckle ``Collection`` hierarchy.

This is the conversion half of the publish flow — the C# ``IBundleBuilder``
contract's "a connector's send is exactly its conversion". Nothing in here
touches the network: ``build_collection_hierarchy`` walks the scene, expands
collection instances and metaball families, converts each object, and returns
the populated root ``Collection`` that ``BlenderBundleExporter`` walks. The
offline harness (``tools/headless_export.py``) calls it directly for exactly
that reason.
"""
...
from .to_speckle import convert_to_speckle
from .instance_unpacker import InstanceUnpackResult, unpack_instances
from .material_to_speckle import add_render_material_proxies_to_base
from .metaball_unpacker import MetaballUnpackResult, unpack_metaballs
