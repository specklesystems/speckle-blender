...
class ReceiveStreamObjects(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if stream.branches.totalCount < 1: return {'CANCELLED'}
        ...
        if len(objects) < 1: return {'CANCELLED'}
        ...
        '''
        scale = get_scale_length(stream_data.units) / context.scene.unit_settings.scale_length 
...
class CreateStream(bpy.types.Operator):
    ...
    stream_description: StringProperty(
        ...
        default="This is a Blender stream.")
    ...
    def execute(self, context):
        ...
        new_stream_id = client.stream.create(name=self.stream_name, description=self.stream_description, is_public=True)
