...
class SpeckleBranchObject(bpy.types.PropertyGroup):
    ...
    def get_commits(self, context):
        if self.commits != None and len(self.commits) > 0:
            return [(str(i), commit.id, commit.message, i) for i, commit in enumerate(self.commits)]
...
class SpeckleStreamObject(bpy.types.PropertyGroup):
    ...
    def get_branches(self, context):
        if len(self.branches) > 0:
            return [(str(i), branch.name, branch.name, i) for i, branch in enumerate(self.branches)]
        return [("0","<none>","<none>", 0)]
...
class SpeckleSceneSettings(bpy.types.PropertyGroup):
    ...
    users: CollectionProperty(
        type=SpeckleUserObject)

    def get_users(self, context):
        return [(str(i), "{} ({})".format(user.email, user.server_name), user.server_url, i) for i, user in enumerate(self.users)]
